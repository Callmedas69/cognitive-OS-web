...
1. Run `npx cognitiveos init` in your project. First question is which AI agent(s) you use (multi-select, all ticked by default), then project type, then your project's name.
2. Open your AI agent (Claude Code, Codex, Antigravity, Cursor). It reads `CLAUDE.md` / `AGENTS.md` and loads your context automatically. First time in, it offers a 60-second, 6-question setup so your project context isn't blank. See [The Keeper](/docs/keeper).
...
- [Commands](/docs/commands): full reference for `init`, `start`, `dump`, `check`, `install-skill`.
